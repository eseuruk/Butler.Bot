...
public class UserChatMessages
{
    ...
    public string SayHello { get; set; } = "Привет! Я помогу тебя зайти в группу <b>{0}</b>. Если не против, я задам тебе несколько вопросов, перед тем как ты сможешь туда попасть.";

    public string SayConfused { get; set; } = "Наверное ты хочешь зайти в группу <b>{0}</b>. Если да, то нажми на кнопку ниже чтоб продолжить.";
    ...
    public string AskForForLeavingConfirmation { get; set; } = "Ты действительно хочешь покинуть группу <b>{0}</b> и удалить свое описание?\n\nЯ не могу удалить все твои сообщения в группе. Если хочешь удалить их, то перейди в группу и сделай это сам перед выходом.";

    public string SayLeavingRequestCanceled { get; set; } = "Запрос на удаление из группы <b>{0}</b> был отменен.";

    public string SayUserLeft { get; set; } = "Если ты состоял в группе <b>{0}</b>, то ты из нее удален. Также удалено твое описание.\n\nПриходи ещё.";
